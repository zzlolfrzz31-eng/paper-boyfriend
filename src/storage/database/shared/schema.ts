...
// 博客文章每日浏览量统计表
export const blogDailyViews = pgTable("blog_daily_views", {
	id: serial().primaryKey().notNull(),
	postId: integer("post_id").notNull().references(() => blogPosts.id),
	viewDate: timestamp("view_date", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	viewCount: integer("view_count").default(0).notNull(),
}, (table) => [
	index("blog_daily_views_post_id_idx").on(table.postId),
	index("blog_daily_views_view_date_idx").on(table.viewDate),
	pgPolicy("blog_daily_views_允许公开读取", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("blog_daily_views_允许公开写入", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("blog_daily_views_允许公开更新", { as: "permissive", for: "update", to: ["public"] }),
]);
